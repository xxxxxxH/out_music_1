...
public class AlbumSongLoader {
    ...
    public static ArrayList<SongsModel> getSongsForAlbum(Context context, long albumID) {

        Cursor cursor = makeAlbumSongCursor(context, albumID);
        ArrayList<SongsModel> arrayList = new ArrayList<SongsModel>();
        if ((cursor != null) && (cursor.moveToFirst()))
            do {
                ...
                arrayList.add(new SongsModel(id, album, albumId, artist, duration,
                        GlobalApp.INSTANCE.getImgUri(albumID), data, title, "", 0, size, trackNumber, artistId, 0));
            }
    ...
    public static Cursor makeAlbumSongCursor(Context context, long albumID) {
        ...
        Cursor cursor = contentResolver.query(uri, new String[]{"_id", "title", "artist", "album", "duration", "track", "artist_id", "_data", "_size"}, string, null, albumSongSortOrder);
        return cursor;
    }
}